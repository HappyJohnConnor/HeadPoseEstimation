...
def Pose_300w():
    ...
    for each_dir in dir_path_ls:
        ...
        for mat_file, jpg_imgae in zip(mat_files, jpg_images):
            pitch, yaw, roll = get_degree_from_mat(mat_file)
...
def AFLW2000():
    ...
    dataset_path =  '../pose_estimation/dataset/test/AFLW2000'
    mat_files = glob.glob(dataset_path +'/*.mat')
    jpg_images = glob.glob(dataset_path +'/*.jpg')

    for mat_file, image in zip(mat_files, jpg_images):
        pitch, yaw, roll = get_degree_from_mat(mat_file)
        ...
        if abs(pitch) <= degree_th and abs(roll) <= degree_th:
            ...
            crop_and_save(mat_file, image, save_path)
