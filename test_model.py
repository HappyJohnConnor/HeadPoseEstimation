...
if __name__ == '__main__':
    ...
    for jpg_image in jpg_images:
        ...
        """
        if abs(designed_deg - np.dot(optical_coef, results[0])) >= 35 and fail_count < 200:
            fail_count += 1
            shutil.copy(jpg_image, './failure_sample/'+ args.direction +'/')
        """
         
    diff_np = np.array(diff_ls)
    print('validiation : %.2f' % np.var(diff_np))
