...
def googlenet(inputs,
    ...
    with tf.name_scope(scope, "googlenet", [inputs]):
        with ops.arg_scope([layers.max_pool2d], padding='SAME'):
            end_points['conv0'] = layers.conv2d(
                inputs, 64, [7, 7], stride=2, scope='conv0')
            end_points['pool0'] = layers.max_pool2d(
                end_points['conv0'], [3, 3], scope='pool0')
            end_points['conv1_a'] = layers.conv2d(
                end_points['pool0'], 64, [1, 1], scope='conv1_a')
            end_points['conv1_b'] = layers.conv2d(
                end_points['conv1_a'], 192, [3, 3], scope='conv1_b')
            end_points['pool1'] = layers.max_pool2d(
                end_points['conv1_b'], [3, 3], scope='pool1')
            ...
            end_points['pool2'] = layers.max_pool2d(
                end_points['inception_3b'], [3, 3], scope='pool2')
            ...
            end_points['pool3'] = layers.max_pool2d(
                end_points['inception_4e'], [3, 3], scope='pool3')
            ...
            end_points['pool4'] = layers.avg_pool2d(end_points['inception_5b'], [
                                                    7, 7], stride=1, scope='pool4')

            end_points['reshape'] = tf.reshape(end_points['pool4'], [-1, 1024])

            end_points['dropout'] = layers.dropout(
                end_points['reshape'], dropout_keep_prob, is_training=is_training)

            end_points['logits'] = layers.fully_connected(
                end_points['dropout'], num_classes, activation_fn=None, scope='logits')

            end_points['predictions'] = tf.nn.softmax(
                end_points['logits'], name='predictions')
